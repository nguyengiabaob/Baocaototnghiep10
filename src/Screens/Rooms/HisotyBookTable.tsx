...
const EditBooktable: React.FC<propschild> =({getvisibleChild})=>{
    ...
    const [time,settime] = useState<Date>(new Date(BookTableSelected.BookDate));
    ...
    const  cancelModal=()=>{
      ...
      const onSave =()=>{
        ...
        if ((time !== undefined && time !== new Date(BookTableSelected.Booktime)) || Slots > 0 )
        {
            if (TableSelected !== undefined)
           
            {data.UpdateBookTable(BookTableSelected.id,BookTableSelected.CustomerName,BookTableSelected.BookDate,time,Slots,TableSelected.id,0).then( res=> console.log(res));
   
            }
            else
            {
                data.UpdateBookTable(BookTableSelected.id,BookTableSelected.CustomerName,BookTableSelected.BookDate,time,Slots,BookTableSelected.TableID,0).then( res=> console.log(res));
            }
...
useEffect(()=>{
    ...
    <View style={{justifyContent:'center', alignItems:'center',marginTop:35}}>
       ...
       <TouchableOpacity onPress={()=>setVisibleTable(true)} style={[styles.Shadowbox,{marginTop:35, alignItems:'center', backgroundColor:'#ffffff', width:reponsivewidth(350),alignSelf:'center',padding:10, }]}>
                <Text>{TableSelected ? TableSelected.Name : BookTableSelected && getNameTable(BookTableSelected) }</Text>
        </TouchableOpacity>
       <View style={{justifyContent:'center', alignItems:'center',marginTop:35}}>
    ...
    <Overlay isVisible={VisibleTable}>
      ...
      <View style={{height:reponsiveheight(250) }}>
        <ScrollView>
          {
            datatable.filter(item=> item.id !== BookTableSelected.TableID && item.Status === 0).map(item=>{
              return (
                <TouchableOpacity key={item.id} onPress={()=>{setTableSelected(item); setVisibleTable(false)}} style={{width:reponsivewidth(320), alignSelf:'center'}}>
      ...
      <View style={{alignItems:'center', marginTop:25}}>
        ...
        <CustomNotification visible={VisibleNotifi} iconTitle={<BellNofi width={reponsivewidth(30)} height={reponsiveheight(30)}/>} title="Thông báo"  onCancel={()=>cancelModal() } Content="Cập nhật thành công !"/>
    </View>
    )
}
const onDelBookTable=()=>{
    ...
    return (
            <View style={{width:getwidth(), height:getheight(),flex:1, marginTop:-9.5}}>
                ...
                <Text style={{alignSelf:'center',fontSize:17, width:reponsivewidth(300), textAlign:'left', color:'#FFFF', fontWeight:'700'}}>Lịch sử đặt bàn</Text>
                </View>
                <ScrollView>
                {dataBooktable.length > 0 && dataBooktable.map(item=>{
                    return (
                <TouchableOpacity onPress={()=>{
                    setBookTableSelected(item);
                    setvisible(true);
                }} style={[ styles.Shadowbox ,{justifyContent:'center',alignItems:'center',width: reponsivewidth(350),alignSelf:'center', paddingRight:25, marginLeft:15, paddingTop:15, paddingBottom:20, marginTop:15, height:reponsiveheight(180),borderLeftColor: item.Status === 1 ? 'red' : '#0d80bf', borderLeftWidth:10, borderRadius:5, }]}>
                ...
                <View style={[{justifyContent:'flex-end',alignItems:'flex-start', width:reponsivewidth(380),marginBottom:10, marginLeft:5 }]}>
                    <Text style={{fontWeight:'700'}}>{tranferday(item.BookDate)}</Text>
                </View>
                <View style={{width:reponsivewidth(280)}}>
                <View style={{flexDirection:'row'}}>
                <Text >Tên khách hàng:</Text><Text style={{marginLeft:5}}>{item.CustomerName}</Text>
                </View>
                ...
                { visible === true &&
                    <EditbooktableCom visible={visible} />
                    }
             <Overlay isVisible={visibleModal}>
                <EditBooktable getvisibleChild={setvisibleModal}/>
             </Overlay>
             <CustomNotificationDel visible={visibleModalDel} iconTitle={<BellNofi width={reponsivewidth(30)} height={reponsiveheight(30)}/>} Content={"Bạn có thực sự muốn xóa lịch đặt bàn này không"} title="Thông báo"  onCancel={()=>setvisibleModalDel(false)} onAction={onDelBookTable}/>
