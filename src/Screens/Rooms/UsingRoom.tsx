...
import { FlatList, SafeAreaView, ScrollView, StyleSheet, Text, Toucac, Touchable, TouchableHighlightComponent, View } from 'react-native';
...
const UsingRooms: React.FC<props> = ({DataArea,DataTable,navigation}:props)=>{
...
useEffect(()=>{
    ...
    return (
        ...
        <View style={styles.ContainerRooms}>
            ...
            scrollsToTop={true}
            scrollEnabled={true}
            data={AreaFilter.id === 'All' ? DataTable.filter(item=>item.Status === 1) : DataTable.filter(item=> item.Type === AreaFilter.id && item.Status === 1)} 
            renderItem={({item})=>(
                <TouchableHighlight onPress={()=>{
                    setvisibleEdit(true);
                    setTableSelected(item.id)
                }} style={[styles.styleRooms , item.Status === 0 ? undefined : {backgroundColor:'#02569E'}  ]}>
                     <Text style={[styles.TitleRooms , item.Status === 0 ? undefined : {color:'#FFF'}]}>{item.Name}</Text>
                </TouchableHighlight>
            )}
            numColumns={3}
            />
