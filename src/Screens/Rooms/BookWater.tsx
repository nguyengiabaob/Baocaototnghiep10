       ...
       .then(async()=>{
        ...
        //     }
            ...
            datatable.forEach(item=>{
                if(item.id === id)
                {
                    data.UpdateTable(item.Name, item.id,item.Type,item.Slots,1).catch(()=>{check=true});
                }
            })
            ProductArray.forEach(item=>{
                data.PostListProduct(item.id,a[0].id,item.Quanity,new Date()).then(res=> {if(res === true)
            ...
            let datapro = await DataService.Getdata_dtService<any>('Products');
                ProductArray.forEach(item=>{
                    datapro.forEach(i=>{
                        ...
                        console.log('array',[item.id, i.name_product, item.Price_product,quanity,i.Image,i.CatergoryID]);
    
                        data.updatedataproduct("Products",item.id,i.name_product,item.Price_product,quanity,i.Image,i.CatergoryID).catch(()=>{check=true});
                        }
    ...
    var arraylist = await DataService.Getdata_dtService<any>('ListProduct');
         arraylist.forEach(item=> {
            ProductArray.forEach( i=> {
                ...
                {
                    data.UpdateListProduct(item.id,item.productID,CodeBill,i.Quanity, new Date()).then(res=> {if(res=== true)
                        {
                ...
                ProductArray.forEach(i=> {
                   if(i.id === item.productID && item.billID ==CodeBill)
                   {
                    datapro1.forEach(j=>{
                        ...
                        data.updatedataproduct("Products",j.id,j.name_product,j.Price_product,quanity,j.Image,j.CatergoryID);
                        }
                    });
                   }
                });
       ...
       // let countadd= 0;
        ProductArray.forEach(item=>{
            ...
            {
                data.PostListProduct(item.id,CodeBill,item.Quanity, new Date ()).then(res=> {if(res === true)
                    {
        ...
        //     }
            ...
            ProductArray.forEach(item=>{
                ...
                datapro1.forEach(i=>{
                    ...
                    {
                    let quanity = Number(i.Quanity)- Number(item.Quanity);
                    data.updatedataproduct("Products",i.id,i.name_product,item.Price_product,quanity,i.Image,i.CatergoryID);
                    }
                });
            }
             ...
             arraylist.forEach(item=>{
                if( item.billID== CodeBill && ProductArray.filter(i=> i.id === item.productID).length === 0 )
                 ...
                 datapro.forEach(i=>{
                     ...
                     data.updatedataproduct("Products",i.id,i.name_product,i.Price_product,quanity,i.Image,i.CatergoryID);
                      }
                 });
             }
...
const ModalChoosenWater:React.FC<propsModalChoosenWater>= ({getdataproductSelected,NewsProducted, codeBill})=>{
    ...
    const [productSelecteddetail,setarrayproductSelecteddetail]= useState<Product>();
   ...
   useEffect(()=>{
    ...
    useEffect(()=>{
        ...
        else
        {
            if(Productbefore)
            {
                setProduct(Productbefore?.filter(item=>item.CatergoryID == CatergorySelected));
            }
        }
    ...
    const searchProduct = useCallback((value: string)=>{
            ...
            {
                console.log();
                if (Product.filter(item=> item.name_product.toLowerCase().search(value.toLowerCase())).length > 0)
                { 
                    setProductsearch(Product.filter(item=> item.name_product.includes(value) ));
    ...
    const BookWaterDetail:React.FC<propsdetail>= ({getdata,ProductSelected, getarrayproduct,codeBill})=>{
        const [valueQuality, setvaluequality]= useState<number>(1);
        return (
            <View style={{width:getwidth(), height:getheight(),flex:1, marginTop:-9.5,backgroundColor:'#e7e7e7'}}>
                <View style={{flexDirection:'row', justifyContent:'center',padding:18, backgroundColor:'#67bff3'}}>
                    ...
                    <Text style={{alignSelf:'center',fontSize:17, width:reponsivewidth(250), textAlign:'center'}}> {codeBill.billID}</Text>
                    <View style={{alignSelf:'flex-start', justifyContent:'flex-start', width:reponsivewidth(50)}}>
                    <TouchableOpacity style={{alignSelf:'flex-end', justifyContent:'flex-end'}} onPress={()=>{
                    if (ProductSelected)
                    {   ProductSelected.Quanity = valueQuality;
                        getarrayproduct(ProductSelected) ;}
                    setvisibleDetail(false);}}>
                    <Text style={{color:'#FFFF', fontWeight:'700', fontSize:16}}>Xong</Text>
                    </TouchableOpacity>
                    </View>
                </View>
                <View style={{flexDirection:'row', alignItems:'center', backgroundColor:'#FFFF', padding:25}}>
                    <Image style={{width: reponsivewidth(80), height: reponsiveheight(80)}} source={{uri:ProductSelected?.Image}}/>
                    <View style={{marginLeft:15}}>
                        ...
                        <Text style={{color:'#67bff3', fontWeight:'700'}}>
                            {ProductSelected?.Price_product.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                        </Text>
                    </View>
                </View>
                <View style={{marginTop:35, backgroundColor:'#FFFF', justifyContent:'center'}}>
                   <View style={{flexDirection:'row', alignItems:'center', padding:15, borderBottomColor:'#c9c9c9', borderBottomWidth:0.75}}>
                       <Text style={{justifyContent:'flex-start', width:reponsivewidth(260) ,fontSize:16}}>Giá bán</Text>
                       <Text style={{justifyContent:'flex-end', backgroundColor:'#e7e7e7', paddingRight:25, paddingLeft:25, paddingBottom:10, paddingTop:10, borderRadius:5,fontWeight:'700'}}>{ProductSelected ? ProductSelected.Price_product.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : 0}</Text>
                   </View>
                   <View style={{flexDirection:'row', alignItems:'center',padding:15,justifyContent:'center'}}>
                       <Text style={{justifyContent:'flex-start', width:reponsivewidth(170), fontSize:16}}>Số lượng</Text>
                       ...
                       <TouchableOpacity onPress={()=>{  if (valueQuality >= 2)
                           {
                           setvaluequality(prev=> prev-1);}
                       }}  style={{justifyContent:'flex-end'}}><AntDesign 
                   ...
                   <View style={{marginTop:40, padding:5, backgroundColor:'#FFFF'}}>
                       <TextInput  placeholder="Ghi chú" style={{padding:10, fontSize:16}}/>
                   </View>
                   <View style={{marginTop:40, padding:20, backgroundColor:'#FFFF'}}>
                       <View style={{flexDirection:'row'}}>
                        <Text style={{justifyContent:'flex-start', width:reponsivewidth(280), fontWeight:'700', fontSize:16}}>Thành Tiền</Text>
                        <Text style={{fontSize:17}}>{ProductSelected ?  (ProductSelected.Price_product * valueQuality).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : 0}</Text>
                        </View>
                   </View>
            </View>
        );
    ...
    return (
        <View style={{width:getwidth(), height:getheight(),flex:1, marginTop:-9.5}}>
            ...
            <View style={{flexDirection:'row', justifyContent:'center',padding:18}}>
                ...
                <Text style={{alignSelf:'center',fontSize:18, width:reponsivewidth(250), textAlign:'center', fontWeight:'700'}}> {bill?.billID}</Text>
                <View style={{alignSelf:'flex-start', justifyContent:'flex-start', width:reponsivewidth(50)}}>
                <TouchableOpacity style={{alignSelf:'flex-end', justifyContent:'flex-end'}} onPress={()=>{
           ...
           <View style={{flexDirection:'row', justifyContent:'center', padding:5}}>
               <SearchBar 
               lightTheme={true}
               containerStyle={{width:reponsivewidth(300),height:reponsiveheight(55),borderRadius:5}}
           ...
           <View>
            ...
            <FlatList style={{backgroundColor:'#529ac5'}}
              scrollEnabled={true}
              horizontal 
              data={[...Arraycatergory,...Catergory]} 
              renderItem={({item})=>{
                  return (
                      <TouchableOpacity onPress={()=>setcatergoryselected(item.id)} style={{padding:10,  borderBottomColor:CatergorySelected === item.id ? '#7ec2eb' : undefined, borderBottomWidth  :CatergorySelected === item.id ? 5 : 0}}>
                          <Text style={{color:CatergorySelected === item.id ? '#FFFF' : '#363636', fontSize:15}}>{item.Name}</Text>
                      </TouchableOpacity>
                  );
              }} />
            }
           ...
           <View style={{marginTop:15}}>
             ...
             <ScrollView style={{height:reponsiveheight(620)}}>
                 ...
                 Productseach.map(item=>{
                  return (
                       <TouchableOpacity onPress={()=>{ setproductSelected(item);setvisibleDetail(true);}} style={{flexDirection:'row',borderBottomColor:'#c1c1c1', borderBottomWidth:0.75, alignItems:'center', padding:15}}>
                       <Image  style={{width: reponsivewidth(80), height: reponsiveheight(80)}} source={{uri:item.Image}} />
                       <View style={{marginLeft:50}}>
                       <Text style={{marginBottom:15, fontSize:18, fontWeight:'bold'}}>{item.name_product}</Text>
                       <Text style={{marginBottom:-10}}>Giá: {item.Price_product.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
                       </View>
                       </TouchableOpacity>
                    );
                 }) 
           ...
           <View style={{flexDirection:'row',alignItems:'center', height:reponsiveheight(80), backgroundColor:'#67bff3'}}>
             ...
             <View style={{alignSelf:'center', justifyContent:'flex-end', flexDirection:'row', alignItems:'center', width:reponsivewidth(360)}}>
                 <Text style={{fontSize:16, color:'#FFFF', marginRight:15, fontWeight:'700'}}>{numberChoosen}</Text>
                 <TouchableOpacity onPress={()=>{
                     if (NewsProducted.length > 0)
                     {
                     ...
                     setvisible(false);
                 }} style={{flexDirection:'row', alignItems:'center'}}><Text style={{color:'#FFFF'}}>Xong</Text><MaterialIcons style={{marginRight:5}} size={25} color={'#FFFF'} name="navigate-next"/></TouchableOpacity>
             </View>
           </View>
           <Overlay isVisible={visbleDetail}>
               <BookWaterDetail  ProductSelected={ProductSelected} getarrayproduct={setarrayproductSelecteddetail} codeBill={bill}/>
           </Overlay>
        </View>
    );
};
...
const ShowPaying: React.FC<propspaying>= ({ListProduct, BillCode,getvisible})=>{
    ...
    <Text style={{alignSelf:'center',fontSize:17, width:reponsivewidth(100), textAlign:'center', color:'#FFFF', fontWeight:'700'}}>Thanh toán</Text>
</View>
<View >
<View style={{flexDirection:'row',paddingBottom :25 , paddingTop:25,paddingLeft:10, paddingRight:10, borderBottomColor:'#d6d6d6', borderBottomWidth:1}}>
    ...
    <View style={{ alignSelf:'flex-end', justifyContent:'flex-end', width:reponsivewidth(240) }}>
        <Text style={{fontSize:18,alignSelf:'flex-end', justifyContent:'flex-end'}}>{databill?.billID}</Text>
    </View>
    </View>
    <View style={{flexDirection:'row',paddingBottom :25 , paddingTop:25,paddingLeft:10, paddingRight:10, borderBottomColor:'#d6d6d6', borderBottomWidth:1}}>
        <Text style={{fontSize:18, marginLeft:10}}>Phương thức</Text>
    <View style={{ alignSelf:'flex-end', justifyContent:'flex-end', width:reponsivewidth(240) }}>
        <Text style={{fontSize:18,alignSelf:'flex-end', justifyContent:'flex-end'}}>Tiền Mặt</Text>
    </View>
    </View>
    <View style={{flexDirection:'row', paddingBottom :25 , paddingTop:25,paddingLeft:10, paddingRight:10, borderBottomColor:'#d6d6d6', borderBottomWidth:1, }}>
    ...
    <View style={{ alignSelf:'flex-end', justifyContent:'flex-end', width:reponsivewidth(270) }}>
        <Text style={{fontSize:18,alignSelf:'flex-end', justifyContent:'flex-end'}}>{isNaN(Number(databill?.Total)) ? 0 : Number(total()).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
    </View>
    </View>
    <View style={{flexDirection:'row', paddingBottom :25 , paddingTop:25,paddingLeft:10, paddingRight:10, borderBottomColor:'#d6d6d6', borderBottomWidth:1, }}>
    ...
    <Text style={{fontSize:18,alignSelf:'flex-end', justifyContent:'flex-end'}}>{CustomerMoney.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
    </View>
    }
    </View>
    <View style={{flexDirection:'row',paddingBottom :25 , paddingTop:25,paddingLeft:10, paddingRight:10, borderBottomColor:'#d6d6d6', borderBottomWidth:1, }}>
        <Text style={{fontSize:18, marginLeft:10}}>Tiền thừa của khách</Text>
    <View style={{ alignSelf:'flex-end', justifyContent:'flex-end', width:reponsivewidth(188) }}>
        <Text style={{fontSize:18,alignSelf:'flex-end', justifyContent:'flex-end'}}>{changeMoney.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
    </View>
    </View>
</View>
</View>
<View style={{backgroundColor:'#67bff3'}}>
    <TouchableOpacity disabled={FinishPaying == true ? true : false} onPress={()=>{CaculateMoney();}} style={{padding:18}}><Text style={{color:'#FFFF', fontWeight:'700', textAlign:'center', fontSize:18} }>Hoàn Thành</Text></TouchableOpacity>
</View>
<CustomNotification visible={showSuccess} iconTitle={<BellNofi width={reponsivewidth(30)} height={reponsiveheight(30)}/>} title="Thông báo"  onCancel={()=>{setShowsuccess(false); getvisible(false); navigation.goBack();}  } Content="Bạn đã thanh toán thành công !"/>
<CustomNotification visible= {showError}  iconTitle={<Warning width={reponsivewidth(30)} height={reponsiveheight(30)}/>} title="Thông báo"  onCancel={()=>setShowError(false) } Content=" Số tiền không đủ thanh toán xin vui lòng nhập lại" />
</View>
    );
};
    ...
    return (
        <View style={{flex:1}}>
            ...
            <View style={[{marginTop:15, backgroundColor:'#FFFF'}, styles.shadowNames ]}>
                <View style={[{ alignItems:'center',flexDirection:'row', padding:12 }]}>
                    <View style={{marginLeft:15}}>
                    <IconTable width={reponsivewidth(100)} height={reponsiveheight(100)} />
                    <Badge
                    badgeStyle={{width: reponsivewidth(11), height: reponsiveheight(11)}}
                     status="success"
                      containerStyle={{ position: 'absolute',bottom: 0, right: 15 }}
                     />
                    </View>
                    <View>
                    <Text style={{marginLeft:35, fontSize:16, fontWeight:'700', marginBottom:25}}>{Table?.Name}</Text>
                    <View style={{alignSelf:'flex-end', justifyContent:'flex-end',marginLeft:20, flexDirection:'row'}} >
                    <TouchableOpacity onPress={()=>{setvisible(true);}} style={{flexDirection:'row', justifyContent:'center', alignItems:'center', backgroundColor:'#226cb3', padding:8, borderRadius:5 }}><Entypo name="add-to-list" size={20} color={'#FFFF'}/><Text style={{color:'#FFFF'}}>Thêm nước</Text></TouchableOpacity>
                    {UpdateProductSelected.length > 0 && CodeBill !=='' ?<TouchableOpacity  onPress={()=>{setvisiblePaying(true);}} style={{flexDirection:'row', justifyContent:'center', alignItems:'center', backgroundColor:'#e86b6b', padding:8, borderRadius:5, marginLeft:10, }}><FontAwesome5 name="money-check-alt" size={15} color={'#FFFF'}/><Text style={{color:'#FFFF'}}>Thanh Toán</Text></TouchableOpacity>
                    : <TouchableOpacity disabled={true}  onPress={()=>{setvisible(true);}} style={{flexDirection:'row', justifyContent:'center', alignItems:'center', backgroundColor:'#c9c9c9', padding:8, borderRadius:5, marginLeft:10, }}><FontAwesome5 name="money-check-alt" size={15} color={'#FFFF'}/><Text style={{color:'#FFFF'}}>Thanh Toán</Text></TouchableOpacity>}
                    </View>
                    </View>
                </View>
            </View>
            <View style={{marginTop:10}}>
                <View style={{justifyContent:'center', alignItems:'center', backgroundColor:'#02569E', padding:10}}>
                    <Text style={{fontSize:16, fontWeight:'700', color:'#FFFF'}}>Danh sách nước</Text>
                </View>
                    <ScrollView style={{height:reponsiveheight(430)}}>
                       {UpdateProductSelected.length >0 ? UpdateProductSelected.map(item=>{
                           return (
                               <View style={{backgroundColor:'#FFFFF', flexDirection:'row', padding: 10, borderBottomColor:'#d6d6d6', borderBottomWidth:1 }}>
                                   <Image style={{width: reponsivewidth(80), height: reponsiveheight(80)}} source={{uri:item.Image}}/>
                                   <View style={{marginLeft:15,width: reponsivewidth(150)}}>
                                   <Text style={{fontSize:17, fontWeight:'700', marginBottom:15}}>{item.name_product}</Text>
                                   <View  style={{flexDirection:'row'}}>
                                   <Text style={{marginRight:15}}> Số lượng  {item.Quanity}</Text>
                                   <Text> Giá : {item.Price_product.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
                                   </View>

                                   </View> 
                                   <View style={{ justifyContent:'center',width:reponsivewidth(110), alignItems:'flex-end'}}>
                                       <TouchableOpacity onPress={()=>{DeleteProduct(item.id);}}>
                                           <MaterialIcons  name="delete" size={32} color={'#999999'}/>
                                       </TouchableOpacity>
                                   </View>
                               </View>
                           );
                    ...
                    <TouchableOpacity onPress={()=>{
                        ...
                        }}   style={{ backgroundColor:'#226cb3',padding:10, width:reponsivewidth(100), justifyContent:'center', alignItems:'center',borderRadius:4, alignSelf:'center'}}>
                ...
                <View>
                    ...
                    <Overlay isVisible ={visible}>
                        <ModalChoosenWater getdataproductSelected={setupdateProducted} NewsProducted={UpdateProductSelected} codeBill={CodeBill}/>
                    </Overlay>
                    : visiblePaying === true &&  <Overlay isVisible ={visiblePaying}>
                    <ShowPaying ListProduct={UpdateProductSelected} BillCode={CodeBill} getvisible={setvisiblePaying}/>
                    </Overlay>
                    }
                </View>
            </View>
            <CustomNotification visible={visibleSuccess} iconTitle={<BellNofi width={reponsivewidth(30)} height={reponsiveheight(30)}/>} title="Thông báo"  onCancel={()=>{setvisibleSuccess(false)}} Content="Bạn đã lưu thành công !"/>
