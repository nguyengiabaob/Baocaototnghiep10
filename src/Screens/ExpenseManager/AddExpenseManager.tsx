...
export const AddExpenseManager: React.FC=()=>{
    ...
    const AddExpense = ()=>{
        ...
        if (ExpenseName === '' || ExpenseContent === '' || ExpenseTotal <= 0 || isNaN(ExpenseTotal))
            {
                setAddError(true);
            }
        else
        {
           data.PostExpense(ExpenseName, ExpenseContent,ExpenseTotal,new Date().toDateString()).then(res=>{
               if (res === true)
               {
                   setAddSuccess(true);
               }
    ...
    return (
        <View>
          ...
          <View style={{marginTop:15, justifyContent:'center', alignItems:'center',  borderBottomWidth:0.5,borderBottomColor:'#c3c3c3',borderTopColor:'#c3c3c3',borderTopWidth:0.5, flexDirection:'row', backgroundColor:'#FFFF',height:reponsiveheight(80)}}  >
                ...
                <TextInput style={[style.textinput,{width:reponsivewidth(220)}]} placeholder="Tên nguồn chi" onChangeText={(text)=>{setExpenseName(text)}}  />
          </View>
          <View style={{justifyContent:'center', alignItems:'center',  borderBottomWidth:0.5,borderBottomColor:'#c3c3c3',borderTopColor:'#c3c3c3',borderTopWidth:0.5, flexDirection:'row', backgroundColor:'#FFFF',height:reponsiveheight(160)}}  >
                ...
                <TextInput   multiline={true} style={[style.textinput,{width:reponsivewidth(220), height: reponsiveheight(150)}]} placeholder="Nội dung nguồn chi" onChangeText={(text)=>{ setExpenseContent(text)}}/>
        </View>
          ...
          <Text  style={style.titlefiled}>Tổng tiền </Text>
                <TextInput keyboardType="numeric"   style={[style.textinput, {width:reponsivewidth(220)}]}  onChangeText={(text)=>{setExpenseTotal(Number(isNaN(Number(text))) ? Number(text.replace(/,/g,'')): Number(text))}}>{ExpenseTotal ? ExpenseTotal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','): 0}</TextInput>
          </View>
