...
import {Image, KeyboardAvoidingView, Platform, SafeAreaView, StyleSheet, TextInput, TouchableOpacity, View} from 'react-native';
...
const AddConfigMaterial: React.FC<props> = ({getArrayItem, ConfigDetail}) => {
...
useEffect(()=>{
  if(ConfigDetail)
  {
    setItemAdding([...ConfigDetail])
  }
},[ConfigDetail])
useEffect(()=>{
  console.log('1234156489789');
  if (ItemAdding || visibleEdit == false)
  {
   getArrayItem([...ItemAdding])
  
  }

},[ItemAdding, getArrayItem,visibleEdit])
                  ...
                  return (
                      <View style={[Styles.shadowContainer, Styles.ContainerBox]} >
                          <View style={{ flex:1}}>
                            {
                              _item.pathimg ? 
                              <Image style={{width: reponsivewidth(80), height: reponsiveheight(80)}} source={_item.Img !== 'none' ? {uri:_item.Img } : {uri:_item.pathimg}  }/>
                              : 
                              <NoImages width= {reponsivewidth(80)} height= {reponsiveheight(80)}/>
                            }
                       ...
                       <TextInput   style={{justifyContent:'flex-end',backgroundColor:'#e7e7e7', borderRadius:5,paddingRight:10, paddingLeft:10, paddingBottom:5, paddingTop:5, fontSize:16,textAlign:'center', marginRight:5}}>{_item.Number}</TextInput>
...
export default AddConfigMaterial;
